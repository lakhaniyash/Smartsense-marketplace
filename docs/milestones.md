...
| **Status notes**  | Catalog Foundation pass complete: Product CRUD (list/detail/create/edit/archive), category browse, cursor pagination, search/filter/sort, and the draft→published→archived status lifecycle are implemented and tested. Per a confirmed scope decision, `ProductVariant`/`Inventory` are created transactionally as internal implementation details (never exposed via the API/UI) and carry no reservation logic — full inventory tracking (this row's original "reservation rules" deliverable) is deferred to alongside M13 Orders, the first real consumer of it. `price` is absent from the schema entirely pending M14's Decimal/Money scalar, mirroring M11's Revenue-placeholder precedent. Category taxonomy management (Admin create/edit/delete) is not built — Partners browse an Admin-seeded tree only. See [TASKS.md § M12 scope notes](./TASKS.md) for the full breakdown. |
