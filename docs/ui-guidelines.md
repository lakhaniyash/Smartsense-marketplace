...
**The page itself never scrolls — only its content region does.** This isn't table-specific: any page whose content can plausibly exceed a viewport (a table, but equally a stack of KPI cards + a chart + a link grid, as on the Reports Dashboard) uses the same bounded-height shell as [Tables § Pagination](#tables) describes — root `flex h-full flex-col`, header/filters as plain non-scrolling children, then the actual content wrapped in `flex flex-1 flex-col overflow-hidden` → `flex-1 overflow-y-auto`, so the header and filter toolbar stay pinned in view while only the content underneath scrolls. A page with a small, fixed amount of content (a detail page like `InvoiceDetailPage`/`OrderDetailPage`/`ProductDetailPage`, showing one entity) is the one legitimate exception — it uses plain page-level scroll since it has no unbounded/potentially-tall content to bound. When adding a new screen, default to the bounded shell unless the content is provably short and fixed-size like a detail page.
...
- **Notifications** is an icon button (`features/notifications`' `NotificationsMenu`, injected into `Header` via its `notificationsSlot` prop from `app/layouts/AppShell.tsx` — `shared/` never imports `features/`, so `Header` itself only knows the slot exists, not what fills it) opening a panel of the caller's own recent notifications, with an unread-count badge polled every 45s (paused while the tab is backgrounded) — no WebSockets/subscriptions (M16, v1.1). A "View all" link opens the full `/notifications` page.
...
| **Breadcrumbs**     | Shown on any page nested more than one level deep from its module root (e.g. Orders → Order #1234) — omitted on top-level list pages where they'd just repeat the sidebar's active item. This applies recursively inside a module with its own internal landing page: Reports' Revenue/Orders/Inventory/Product Performance/Notification Activity/Billing Reports pages are each the direct, one-level-deep entry point for their report (reached from the Reports dashboard's link grid, the same relationship Catalog/Orders/Billing's list pages have to the sidebar) — no crumb on any of them, only on the further-nested `/reports/billing-reports/:id` detail page. Getting this wrong isn't just a visual nit: an unwanted crumb adds real height inside `Content`'s bounded `main` region that a page's own `flex h-full flex-col` doesn't reserve room for, silently breaking the [Page Layouts](#page-layouts) bounded-scroll shell (found on M15 Reports, fixed by removing the 6 unwarranted crumbs). Derived from route metadata (`handle.crumb` in `app/router/index.tsx`), rendered once by the shell (`shared/layouts/Breadcrumbs.tsx`) — no page hand-rolls its own `<Breadcrumb>`. A page with a data-dependent trailing label (a Product's title, an Order's number) calls `useBreadcrumb(label)` to replace the route's static fallback once the label loads; `undefined` (still loading) leaves the fallback in place rather than showing a blank crumb.                                        |
...
| **Back navigation** | **`window.history.back()` is never used anywhere in this app, for any back-style control** — not just the Pagination "Previous" button ([Tables § Pagination](#tables)'s cursor-stack fix), but also a detail page's "Back to X" action (e.g. on an `ErrorState`'s `action` slot). Browser history is unreliable the moment a page is reached via a bookmarked/shared URL, a filter change pushed its own entry, or the user arrived from outside the app's own navigation — in every one of those cases "back" silently does the wrong thing (or leaves the app) instead of failing loudly. Always navigate to a concrete, known destination instead: a data hook's own client-side cursor stack for pagination (`useInvoices`/`useCatalog`/`useOrders`/`useNotifications`/every `features/reports` list hook all follow this), or a plain `Link` to the feature's list route (styled to match `Button`'s secondary/md variant, per `dashboard/components/QuickActions.tsx`'s existing "`Link` styled as a button" precedent — a real anchor is correct here since this is navigation, not a form action) for a detail page's "Back to X". This was fixed once for pagination (SM-244) and again for detail-page back-buttons across Catalog/Billing/Orders/Reports when the same anti-pattern recurred — treat this table row as the standing rule so it doesn't need rediscovering a third time: **any new screen with a "back" or "previous" control must use one of these two patterns, never browser history.** |
...
| **Notifications**     | Asynchronous, user-directed events not tied to the current in-progress action (an "Order confirmed"/"Invoice issued" notification)                                                                                                 | Real as of M16 (v1.1): `features/notifications`' header dropdown ([Header Behavior](#header-behavior)) + a full `/notifications` page, populated by domain events (Orders' 4 M13 events, Billing's `invoice.generated`/`payment.recorded`) — never written directly, only via `NotificationEventsListener`. Email delivery remains a v1.1 follow-up, tracked in [Future Enhancements](#future-enhancements). |
...
- **Notifications email delivery** — the in-app notification center ([Header Behavior](#header-behavior), [Feedback Components](#feedback-components)) shipped in M16; email delivery for the same event set is the remaining v1.1 piece ([roadmap.md § Notifications](./roadmap.md#notifications)), tracked as `M16-T3` ([TASKS.md](./TASKS.md)), not yet built.
