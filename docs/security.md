...
1. **CORS fail-open in production is now closed**: `apps/api/src/main.ts` restricts CORS to a configured origin allowlist (`CORS_ALLOWED_ORIGINS`) when one is set, and still falls back to a permissive, any-origin policy when it is unset — but that fallback is now reachable **only outside production**. The Joi schema (`apps/api/src/config/validation.schema.ts`) requires a non-empty `CORS_ALLOWED_ORIGINS` when `NODE_ENV=production`, mirroring the existing `GRAPHQL_INTROSPECTION` fail-closed pattern, so a production boot with no allowlist aborts at config validation instead of silently starting wide open (v1.0 Release Readiness Audit finding F-M1). Local dev and CI keep the permissive default unchanged — see [CORS](#cors).
...
| A05 | Security Misconfiguration                | **Improving**: CORS no longer fails open in production — the Joi schema requires `CORS_ALLOWED_ORIGINS` when `NODE_ENV=production` (F-M1 closed); the permissive fallback remains only for dev/CI. The remaining residual is the dev realm's dev-grade Keycloak settings, listed in the gap register with an owner ([Assumptions](#purpose)). Security headers and CSP are served (helmet on the API, a full CSP + headers at the nginx SPA layer). Boot-time config validation prevents the silent-misconfiguration variant. |
...
**Fail-closed in production** (gap #1, closed): `apps/api/src/main.ts` reflects any origin only when `CORS_ALLOWED_ORIGINS` is unset — a convenience now confined to non-production. The posture:

- Allowed origins are configuration (`CORS_ALLOWED_ORIGINS`, comma-separated, validated by Joi like everything else) — exact origins per environment, no wildcards outside local dev, mirroring the exact-match redirect-URI rule ([authentication.md](./authentication.md#security-best-practices-checklist)).
- The Joi schema requires a non-empty `CORS_ALLOWED_ORIGINS` when `NODE_ENV=production` (same `.when('NODE_ENV', …)` fail-closed shape as `GRAPHQL_INTROSPECTION`), so a production boot without an allowlist aborts at config validation rather than starting with an open policy. Dev/CI keep the permissive default.
- `credentials` stays disabled by default — auth is header-based, no cookies cross this boundary ([CSRF](#csrf)); it is enabled only alongside an explicit allowlist.
- Remaining work: setting the actual production origin value(s) is part of environment provisioning (M20-T3). The code-level fail-open gap this register previously tracked is closed (F-M1).
