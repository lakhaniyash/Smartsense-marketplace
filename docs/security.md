...
1. **CORS is wide open**: `app.enableCors()` runs with no options (`apps/api/src/main.ts`), allowing any origin. Acceptable for local development only — see [CORS](#cors).
2. **No security headers / CSP**: no helmet-style middleware on the API, no serving layer (and therefore no headers) for the frontend yet — see [Security Headers](#security-headers), [CSP](#csp).
...
| A05 | Security Misconfiguration                | **The current weak area, knowingly**: open CORS, no headers/CSP, dev-grade Keycloak settings — all listed in the gap register with owners ([Assumptions](#purpose)). Boot-time config validation prevents the silent-misconfiguration variant.                               |
