...
| Test user (Admin role)   | `yash.lakhani+admin@smartsensesolutions.com` / `Admin@12345`     |
| Test user (Partner role) | `yash.lakhani+partner@smartsensesolutions.com` / `Partner@12345` |
...
| ESLint auto-fix "breaks" a NestJS service import (`import { type X }` on an injected class) | Should not happen — `consistent-type-imports` is enabled in `packages/eslint-config/api.cjs` (inline `type` fixStyle) but does not rewrite injected-class imports; type-only imports on DI classes break Nest. If you see it, your editor is using a different ESLint config; fix the editor setup, don't commit the change. |
