...
| `KEYCLOAK_URL`                                                   | `http://localhost:8080`                                                             | Yes (new)                                                                            | Base URL the API itself uses to reach Keycloak (JWKS fetch, Admin API) — the container-network hostname in Docker Compose, e.g. `http://keycloak:8080`.                                                                                                                                                           |
| `KEYCLOAK_ISSUER`                                                | `http://localhost:8080`                                                             | No (defaults to `KEYCLOAK_URL`)                                                      | Browser-facing base URL used to construct the expected `iss` for token validation. Required whenever it differs from `KEYCLOAK_URL` — Docker Compose's full stack is exactly this case: the API reaches Keycloak via the service name `keycloak`, but issued tokens carry `iss` based on the hostname the browser actually logged in through (`localhost`). Left unset (or equal to `KEYCLOAK_URL`), every authenticated request fails closed with `UNAUTHENTICATED` on an issuer mismatch — see `infrastructure/docker/docker-compose.yml`'s `api` service for the working example, including the paired `KEYCLOAK_JWKS_URI` override this requires (JWKS's own default is derived from the issuer, so it needs pinning too once the issuer becomes browser-facing). |
...
| `CORS_ALLOWED_ORIGINS`                                           | _(empty)_                                                                           | No (has default) — **set in production**                                             | Comma-separated browser origin allowlist (SM-269). Empty ⇒ permissive CORS (local dev); in production set to the web app origin(s), e.g. `https://app.smartsense.example`, so the API only reflects CORS for known callers. |
