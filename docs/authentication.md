...
flowchart LR
    ...
    subgraph IdP["Keycloak (trusted identity provider)"]
        KC["Realm: smartsense<br/>Client: marketplace-app (public, PKCE)<br/>Client: marketplace-api (bearer-only)"]
    end
...
| Object          | Value                        | Why                                                                                                                                                                               |
| --------------- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Realm           | `smartsense`                 | One realm for the whole platform; Admin/Partner/Customer are realm roles, not separate realms — they share one user directory and one login screen.                               |
| Frontend client | `marketplace-app`            | Public client, **Authorization Code + PKCE** flow, no client secret (secrets can't be kept in a browser bundle — `docs/architecture.md`: "Do not store secrets in the frontend"). |
| Backend client  | `marketplace-api`            | Confidential, **bearer-only** client — it never initiates a login, it only validates tokens issued for `marketplace-app`. Used as the JWT audience.                               |
...
flowchart TD
    ...
    I -- No --> Z1
    I -- Yes --> J{"aud / azp includes<br/>marketplace-api?"}
    J -- No --> Z1
...
| `aud` / `azp` | Must include/equal the backend's client id (`marketplace-api`)                                             | Prevents a token issued for a different client/application from being accepted here (audience confusion).                         |
...
| `VITE_KEYCLOAK_REALM`                | `smartsense`                    | Yes (existing) | Realm name.                                                                                                                                    |
| `VITE_KEYCLOAK_CLIENT_ID`            | `marketplace-app`               | Yes (existing) | Public SPA client id.                                                                                                                          |
...
| `KEYCLOAK_REALM`                                                 | `smartsense`                                                            | Yes (new)                                                                            | Realm whose issuer/JWKS the API trusts.                                                                                                                                                                             |
| `KEYCLOAK_API_CLIENT_ID`                                         | `marketplace-api`                                                       | Yes (new)                                                                            | This API's own client id — expected token audience (`aud`/`azp`).                                                                                                                                                   |
...
Local dev bootstrap order implied by these dependencies: `keycloak-db` → `keycloak` → (realm import) → `db` → `api` → `web`. Realm/client import (the `smartsense` realm, `marketplace-app`/`marketplace-api` clients, seeded via a Keycloak realm export JSON mounted into the container) is a one-time provisioning concern for the implementation phase, not modeled further here.
