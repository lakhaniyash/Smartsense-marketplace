...
| `Order.subtotal/tax/shippingCost/total >= 0`                                                   | `Order`          | Four `CHECK` constraints (`orders_subtotal_non_negative_check`, `orders_tax_non_negative_check`, `orders_shipping_cost_non_negative_check`, `orders_total_non_negative_check`)         |
| `Invoice.amountDue >= 0`                                                                       | `Invoice`        | `CHECK` constraint `invoices_amount_due_non_negative_check`                                                                                                                            |
| `Payment.amount >= 0`                                                                          | `Payment`        | `CHECK` constraint `payments_amount_non_negative_check`                                                                                                                                |

Verified against a disposable PostgreSQL 17 database: all thirteen constraints correctly reject invalid inserts/updates (overlapping billing periods, out-of-range commission rate, inconsistent owner-type/FK combination, duplicate default address per owner+type, a second default Variant per Product, a non-positive Variant price, a negative Order/Invoice/Payment amount) while valid rows still succeed.

Trigram search indexes (`20260722_add_pg_trgm_search_indexes`, v1.0 Release Readiness Audit finding F-H5) are a hybrid of the two mechanisms: the four GIN indexes themselves — `customers_display_name_idx`, `customers_billing_email_idx`, `users_email_idx`, `users_full_name_idx` — are declared _declaratively_ in `schema.prisma` (`@@index([<field>(ops: raw("gin_trgm_ops"))], type: Gin)` on `User` and `Customer`), because Prisma 6 can model GIN indexes with an operator class and doing so keeps `prisma migrate` drift-free. What Prisma cannot manage without the `postgresqlExtensions` preview feature is the `pg_trgm` extension those indexes depend on, so `CREATE EXTENSION IF NOT EXISTS pg_trgm;` is hand-written at the top of that migration — exactly the `btree_gist` pattern above. They back `CustomersService`/`UsersService` `buildWhere`'s case-insensitive `ILIKE '%term%'` search, which a plain B-tree can never serve because of the leading wildcard. Verified with `EXPLAIN ANALYZE` on a ~100k-row synthetic dataset: a selective search flips from a `Seq Scan` (~55 ms) to a `Bitmap Index Scan` on the new GIN index (~0.6 ms); the synthetic rows were removed afterward, leaving the seed baseline unchanged.

`Order.total`, `Product.title`, and `OrderStatusHistory.changedByUserId` also gained plain (non-partial) indexes in the same migration — `OrderSortField.TOTAL` and `ProductSortField.NAME` are both client-sortable columns that had no backing index, so an Admin caller (no partner/customer filter narrowing the row count) sorting either full list forced a sequential scan; `changedByUserId` had no index of its own for looking up an actor's history. `orders.service.ts`'s and `catalog.service.ts`'s `buildOrderBy` also now append `id` as a compound tiebreaker on every sort order — two rows sharing the same total/title/createdAt could otherwise skip or repeat across cursor-paginated pages, since Prisma's cursor pagination requires `orderBy` to fully determine a total order.
