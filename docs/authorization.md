...
| **Partner**                | Run their own selling operation: catalog, inventory, order fulfillment, billing visibility                                  | Dashboard, Catalog, Orders, Billing (read), Reports (own), Settings when shipped                                 | Strictly ownership-scoped: only records where `partnerId` matches their organization; no user administration; no billing management.                                                                |
