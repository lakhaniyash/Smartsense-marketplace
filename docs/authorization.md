...
self-service (a Customer already manages their own read-mostly experience via `orders:read`/
`orders:create`, unrelated to this admin surface).
...
| **Partner**                | Run their own selling operation: catalog, inventory, order fulfillment, billing visibility                                  | Dashboard, Catalog, Orders, Billing (read), Reports (own), Customers (own, Sprint 2), Settings when shipped      | Strictly ownership-scoped: only records where `partnerId` matches their organization (Customers: only ones with a shared Order — see [Ownership Rules](#ownership-rules)); no user administration; no billing management. |
...
| **Reports**                                                    | Generate/view billing reports; view Revenue/Orders/Inventory/Product Performance/Notification Activity/Customers (SM-330) reports and the Reports Dashboard | `reports:read`                                                                               | All partners | Own             | —                                                                                                       |
...
|                                                                | Add / edit / deactivate a customer's addresses (SM-324)                                                                                                     | `customers:write`                                                                            | All          | Own (see below) | —                                                                                                       |

`reports:read` (M15) is now seeded — a single key covers view _and_ generate for every report type in
this table's Reports row (no separate `reports:manage`). When M17 introduces `settings:manage`, this
table and the [seeded catalog](#the-seeded-catalog) are updated in the same PR as the migration that
adds it, the same way M15's permission landed.

**Customers is a Sprint 2 initiative (Jira Epic SM-320), not a `docs/milestones.md` roadmap
milestone** — it does not get an `M*` number. `Customer` has no `partnerId` FK, so "Partner: Own"
in this row means something structurally different from every other row above it: a Partner's
"own" Customers are those with **at least one Order placed with that Partner as vendor**, derived
through `Order`, not a direct ownership column ([Ownership Rules](#ownership-rules)). Creating a
brand-new Customer has no such Order to derive a floor from, so it is Admin-only — a Partner may
edit/suspend/reactivate a Customer only once "own" via a shared Order, per the same table's Edit
row. A Partner's billing summary for a shared Customer is further scoped to invoices where
...
| Partner  | Customers _(Sprint 2)_                  | Only Customers with at least one Order where `Order.partnerId == user.partnerId` — derived through Order, since `Customer` has no `partnerId` FK. Cannot create a Customer (no Order exists yet to derive a floor from — Admin-only). Billing summary further scoped to `Invoice.partnerId == user.partnerId`. |
...
| Customer | Customers _(Sprint 2)_                  | No access — `customers:read`/`customers:write` are not granted to the Customer role; this is an Admin/Partner management surface, not buyer self-service                                                                                                                                                       |
...
- **Planned permission keys** — `settings:*` (M17) per [Resource Authorization](#resource-authorization); **Super Admin** and **Read-only** roles per [System Roles](#system-roles). These are scheduled evolution of the current model, not new mechanisms.
