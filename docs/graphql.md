...
| **Scalars**        | Built-in scalars (`String`, `ID`, `Boolean`, `Int`, `Float`, the built-in `DateTime` for `Date`-typed fields) plus one custom scalar, `Decimal` (`apps/api/src/common/graphql/decimal.scalar.ts`, registered globally via `CommonModule`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `ID` for every entity identifier (matches `Prisma`'s UUID primary keys, [database-schema.md](./database-schema.md#design-conventions)), never a raw `String` for a field that is structurally an identifier. `Decimal` for every money/percentage field — never `Float`. |
...
| **Custom Scalars** | `Decimal` (M12) — serializes/parses as a string to avoid floating-point precision loss; used by `ProductVariant.price`, `CreateProductInput.price`, `CreateProductVariantInput.price`, `UpdateProductVariantInput.price`. `Decimal`-typed Input DTO fields are declared `string` at the TypeScript level, not `Prisma.Decimal` — NestJS's global `ValidationPipe` (class-transformer's `plainToInstance`) unconditionally calls `new (reflected design:type)()` on any nested non-primitive object without an explicit `@Type()`, which crashes for a real `Decimal` instance (its constructor rejects zero arguments); `string` is one of class-transformer's safe special-cased primitives, and Prisma's generated types already accept a plain string anywhere a `Decimal` column is written. | Reused as-is by Orders (M13) and Billing (M14) — introduce a second custom scalar only for a genuinely different representation need, not per-module.                                                                                                                    |
