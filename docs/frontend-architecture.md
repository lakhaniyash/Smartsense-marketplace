...
**Assumption made explicit.** The application shell is real but young: `main.tsx` → `App` → `Providers` (Apollo + Auth) → `AppRouter` (real route tree with guards) exists and runs. `app/bootstrap`, `app/guards`, and `app/layouts` are implemented as of M8 (frontend authentication) — see [Authentication Flow](#authentication-flow) and [Routing Architecture](#routing-architecture). The `dashboard`/`catalog`/`orders`/`billing` feature folders still hold only thin placeholder pages proving the M8 routing/guard infrastructure; their real UI is scaffolded pending milestones M9–M14 ([milestones.md](./milestones.md#milestone-details)). This document defines the architecture those milestones fill in — sections describing not-yet-built behavior name the milestone that builds it.
...
| 3   | Initialize authentication (Keycloak adapter, silent SSO check)                 | `app/bootstrap` + auth service | ✅ Implemented (M8) — `AuthProvider` renders the app-level loading state until this resolves, so the app never flashes login-then-content or content-then-login ([authentication.md § Route Protection](./authentication.md#route-protection-frontend)) |
| 4   | Construct Apollo Client (auth + error links need the auth service from step 3) | `lib/apollo`                   | ✅ Implemented (M8) — `lib/apollo/links/authLink.ts` and `errorLink.ts`                                                                                                                                                                                 |
