...
// § State Management Strategy), same pattern as useCatalog/useOrders.
export function useCustomers() {
  const [searchParams, setSearchParams] = useSearchParams()

  const search = searchParams.get('search') ?? undefined
  const status = readOptionalEnumParam(searchParams, 'status', Object.values(CustomerStatus))
  const after = searchParams.get('after') ?? undefined
  // Client-side cursor stack for "Previous" — see useCatalog's identical
  // comment on why this is never the browser's history.
  ...
  const { data, loading, error, refetch } = useQuery(GetCustomersDocument, {
    variables: {
      ...
      after: after ?? null,
      filter,
      sort: { field: CustomerSortField.DisplayName, direction: SortDirection.Asc },
    },
  })

  function setFilter(key: 'search' | 'status', value: string | undefined) {
