...
import { ReportFilterBar, RevenueTrendChart } from '../components'
...
// ExportMenu's own `ExportFormat` union ('csv' | 'excel') maps to the
// schema's `ReportExportFormat` enum right here — ExportMenu itself has no
// GraphQL knowledge (its own doc comment), so this mapping is this page's
// job, not a shared one (each report page repeats this one-line table).
...
export function RevenueReportPage() {
  const { report, isLoading, error, filters, setDateRange, refetch } = useRevenueReport()
  ...
  async function handleExport(format: ExportFormat) {
    ...
    try {
      const csv = await exportReport({
        reportType: ReportExportType.Revenue,
      ...
      toast({
        title: "Couldn't export the revenue report",
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          <div className="flex flex-1 flex-col gap-6 overflow-y-auto">
            <div className={KPI_GRID_CLASSES}>
              <KpiCard
                label="Gross revenue"
                value={Number(report.totalGrossRevenue)}
                format="currency"
              />
              <KpiCard
                label="Commission"
                value={Number(report.totalCommission)}
                format="currency"
              />
              <KpiCard label="Net payout" value={Number(report.totalNetPayout)} format="currency" />
              <KpiCard label="Invoices" value={report.invoiceCount} format="count" />
            </div>
            <RevenueTrendChart trend={report.trend} />
          </div>
        </div>
      )}
    </div>
