...
import { OrdersTrendChart, ReportFilterBar } from '../components'
import { toDateRangeInput } from '../constants'
import { useOrdersReport } from '../hooks'
...
export function OrdersReportPage() {
  const { report, isLoading, error, filters, setDateRange, refetch } = useOrdersReport()
  ...
  async function handleExport(format: ExportFormat) {
    ...
    try {
      const csv = await exportReport({
        reportType: ReportExportType.Orders,
      ...
      downloadBlob(new Blob([csv], { type: 'text/csv' }), 'orders-report.csv')
    } catch (exportError) {
      toast({
        title: "Couldn't export the orders report",
  ...
  return (
    <div className="flex h-full flex-col gap-6">
      ...
      {!isLoading && error !== undefined && (
        <ErrorState
          title="Couldn't load the orders report"
      ...
      {!isLoading && error === undefined && report !== undefined && (
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          <div className="flex flex-1 flex-col gap-6 overflow-y-auto">
            <div className={KPI_GRID_CLASSES}>
              <KpiCard label="Total orders" value={report.totalOrders} format="count" />
              <KpiCard
                label="Total revenue"
                value={Number(report.totalRevenue)}
                format="currency"
              />
              <KpiCard
                label="Average order value"
                value={Number(report.averageOrderValue)}
                format="currency"
              />
            </div>
            <OrdersTrendChart statusBreakdown={report.statusBreakdown} />
          </div>
        </div>
      )}
    </div>
