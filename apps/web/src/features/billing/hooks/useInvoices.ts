...
// URL-backed filter/sort/cursor state, same pattern as useOrders
// (docs/frontend-architecture.md § State Management Strategy).
export function useInvoices() {
  const [searchParams, setSearchParams] = useSearchParams()

  const status = readOptionalEnumParam(searchParams, 'status', Object.values(InvoiceStatus))
  const sortField = readEnumParam(
    searchParams,
    'sortField',
    Object.values(InvoiceSortField),
  ...
  const { data, loading, error, refetch } = useQuery(GetInvoicesDocument, {
    variables: {
